...
export async function ingestMenuItem(item) {
  ...
  console.log('[rag] ingestMenuItem', {
...
export async function queryMenuStream({ restaurantId, question }) {
  ...
  console.log('[rag] queryMenuStream', { restaurantId, question });
  ...
  console.log('[rag] matches', matches.length, matches.map((m) => ({
    score: m.score,
    name: m.metadata?.name,
    restaurantId: m.metadata?.restaurantId,
    menuId: m.metadata?.menuId,
  })));
  ...
  const systemPrompt = `You are a helpful assistant for a restaurant. Answer questions about the menu based only on the context provided. The context lists every relevant menu item with its fields (name, price, description, calories, dietaryTags, allergens, ingredients, isAvailable, isFeatured). When the user asks for "featured" items, only include items where isFeatured is true. When asked for the full menu or all items, list every item in the context. Be concise and friendly. If the answer is not in the context, say you don't have that information.`;
